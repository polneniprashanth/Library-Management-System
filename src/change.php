...
<?php
    ...
    if(isset($_POST['Submit'])){
        ...
        if($roww["password"]!=$_POST["pass"]){
            header("Location:http://upgradetopro.great-site.net/LMS/change.php?msg1=incorrect password!!!");
            echo "incorrect password";
        }
        if($_POST["npass"]!=$_POST["cnpass"]){
            header("Location:http://upgradetopro.great-site.net/LMS/change.php?msg1=Passwords didn't match!!!");
            echo "Passwords didn't match"; 
        }
        if(strlen($_POST['npass'])<8){
            header("Location:http://upgradetopro.great-site.net/LMS/change.php?msg1=Password length is short!!!"); 
            echo "Password length is short";
        }
        mysqli_query($conn,"UPDATE register set password='" . $_POST["npass"] . "' WHERE Id='" . $id . "'");
        header("Location:http://upgradetopro.great-site.net/LMS/home.php?msg1=Password changed successfully!");
    }
?>
<!DOCTYPE html>
<html lang="en">
